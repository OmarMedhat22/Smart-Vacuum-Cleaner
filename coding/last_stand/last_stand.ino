/*
firstly the vacuum cleaner go to any coner it detect ,it programed so that it must find corner which make the right sensor (sensor 2) have distance > wall_distance
secondly the vacuum cleaner will move forward if it detect distance < wall_distance it will turn_left and move forward for 1 second then turn left again and move forward
thirdly  the vacuum cleaner will move forward if it detect distance < wall_distance it will turn_right and move forward for 1 second then turn right again then move forward and the cycle repeat it self
fourthly the vacuum cleaner will detect any door and save this place in 2D array (x_axis,y_axis)
