...
void check(boolean check_left_or_right){
  distance1 = ultra_sonic();
  if(distance1 < wall_distance)
  {
    if(check_left_or_right == false){
      distance3 = ultra_sonic();
    if(distance3 <= wall_distance){
      end_moving(check_left_or_right);
     }
    }
    else{      
...
void end_moving(boolean check_left_or_right){
  if(check_left_or_right){
    while(1){
    move_backward(); // lsa m3mlthash
    distance2 = ultra_sonic();
    if(distance2 > wall_distance){
      break;
      }
    else if(x_axis > x_y_1[0][0]){
      while(1){
        stop_moving();
        }
      }
